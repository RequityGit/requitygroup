...
import { US_STATES, ENTITY_TYPES, stripAddressFields } from '../../lib/format';
...
export function EntitiesTab({ borrowerId, entities, onRefresh, showToast }) {
  ...
  async function handleSave(e) {
    ...
    const cleanPayload = stripAddressFields(payload);

    let error;
    if (editingId) {
      const res = await supabase.from('borrower_entities').update(cleanPayload).eq('id', editingId);
      error = res.error;
    } else {
      const res = await supabase.from('borrower_entities').insert([cleanPayload]);
      error = res.error;
    }
